#!/usr/bin/env bash
# Builds ChampSim binaries for seznec_tagesc size variants.
# Uses isolated build directories so multiple instances of this script can run in parallel.
#
# Usage:
#   ./build_all_tagesc.sh [-j jobs] [-v variant] [-o output_dir] [-p build_root]
# Examples:
#   ./build_all_tagesc.sh
#   ./build_all_tagesc.sh -v 192kb -j 8
...
# Resolve user-provided paths to absolute paths so builds are independent of cwd.
if [[ "$OUTPUT_DIR" = /* ]]; then
...
for VARIANT in "${VARIANTS[@]}"; do
  CONFIG="champsim_config_seznec_tagesc_${VARIANT}.json"
  FINAL_BINARY="${ABS_OUTPUT_DIR}/champsim_seznec_tagesc_${VARIANT}"
  VARIANT_ROOT="${ABS_BUILD_ROOT}/seznec_tagesc_${VARIANT}"
  ...
  # Ensure each variant build starts clean; avoids stale build-id/object mismatches.
  rm -rf "$VARIANT_ROOT"
  mkdir -p "$VARIANT_BINDIR" "$VARIANT_MAKEDIR"

  echo "============================================================"
  echo "  Building variant: seznec_tagesc_${VARIANT}"
  echo "  Config:     $CONFIG"
...
ls -lh "${ABS_OUTPUT_DIR}"/champsim_seznec_tagesc_* 2>/dev/null || echo "  (none found)"
